...
const ProductManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    tabbedSections: [],
    variants: [
      { id: Date.now(), name: "", price: "", salePrice: "", stock: "", sku: "", barcode: "" },
    ],
  });
  ...
  const deleteResultImage = (index) => {
    const imageToDelete = formData.resultImages[index];
    let newResultFiles = [...(formData.resultFiles || [])];
    if (typeof imageToDelete === "string" && !imageToDelete.startsWith("http")) {
      const newImageIndex = formData.resultImages
        .slice(0, index)
        .filter((img) => typeof img === "string" && !img.startsWith("http")).length;
      newResultFiles = newResultFiles.filter((_, i) => i !== newImageIndex);
    }
    setFormData({
      ...formData,
                  ...
                  {modalTab === "results" && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-2 duration-300">
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          {(formData.resultImages || []).slice(0, 4).map((img, idx) => (
                            <div
                              key={`${img}-${idx}`}
                          ...
                          {Array.from({ length: Math.max(0, 4 - (formData.resultImages || []).length) }).map((_, idx) => (
                            <div
                              key={`upload-${idx}`}
                              className="aspect-square rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 flex flex-col items-center justify-center group hover:border-primary hover:bg-primary/5 transition-all cursor-pointer overflow-hidden relative">
                              <input
                        ...
                        <p className="text-[10px] text-slate-500 font-medium">
                          Existing results images are shown here. Uploading new images will append them to the list.
                        </p>
                      </div>
                    </div>
                  )}
                      ...
                      <div className="space-y-3">
                        {formData.variants.map((v, i) => (
                          <div key={v.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                            ...
                            <div className="col-span-6 grid grid-cols-1 md:grid-cols-12 gap-4 items-end mt-2 pt-2 border-t border-slate-200/50">
                              ...
                              <div className="col-span-12 md:col-span-6 flex gap-2">
                                <button
                                  ...
                                  className="flex-1 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1"
                                >
                                  Scan
                                </button>
                                <button
                                  ...
                                  className="flex-1 px-3 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1"
                                >
                                  Generate
                                </button>
                              </div>
