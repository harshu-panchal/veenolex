...
export default function AdminPOSPaymentSuccess() {
  ...
  // Obtain merchantOrderId or orderId from redirect parameters
  ...
  useEffect(() => {
    if (orderId) {
      ...
      setLoading(false);
      toast.error("No transaction order identifier found in redirect URL");
    }
  }, [orderId]);
  ...
  const verifyPayment = async () => {
    try {
      setLoading(true);
      const res = await adminPosApi.verifyOnlinePayment(orderId);
      if (res.data?.success) {
        setOrder(res.data.order);
        setSuccess(true);
        toast.success("PhonePe payment received and confirmed!");
      } else {
        setSuccess(false);
      ...
      <div className="w-full max-w-sm bg-zinc-900 border border-zinc-850 rounded-2xl p-6 text-center shadow-2xl">
        {loading ? (
          <div className="py-12 space-y-4">
            ...
            <p className="text-zinc-400 text-sm">Verifying transaction with gateway...</p>
          </div>
        ) : success ? (
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-rose-400 bg-rose-500/10 border border-rose-500/20 p-3 rounded-xl mt-4">
                The payment attempt failed, cancelled, or was declined by the customer's bank.
              </p>
            </div>
